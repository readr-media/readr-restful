...
func TestRouteProjects(t *testing.T) {
	...
	t.Run("PutProject", func(t *testing.T) {
		testcases := []genericTestcase{
			...
			genericTestcase{"UpdateMemoSchedule", "PUT", "/project/schedule/publish", `{"id":32768,"publish_status":3,"published_at":"2046-01-05T00:42:42+00:00"}`, http.StatusOK, ``}, //published_at is time string in RFC3339 format
		}
		for _, tc := range testcases {
			genericDoTest(tc, t, asserter)
		}
